...
class NDBDataProcessor:
    ...
    def transform_data(self, df):
        ...
        try:
            ...
            if 'CELL_SYSTEM_INFO' in transformed_df.columns:
                transformed_df['Fixed_Ant_Size'] = transformed_df['CELL_SYSTEM_INFO'].apply(get_fixed_ant_size)
            else:
                transformed_df['Fixed_Ant_Size'] = 0.03
            ...
            if 'CELL_NAME' in transformed_df.columns:
                transformed_df['Class_Cell'] = transformed_df['CELL_NAME'].apply(extract_class_cell)
            else:
                transformed_df['Class_Cell'] = ""
    ...
    def filter_allowed_columns(self, df):
        """Filter kolom yang diperbolehkan (sesuai macro VBA)"""
        try:
            log_message("START", "Filtering kolom yang diperbolehkan...")
            
            # Kolom yang diperbolehkan (sesuai macro VBA)
            allowed_columns_raw = [
                ...
                "Cell_Class", "CELL_SYSTEM_INFO", "SECTORID", "SectorID", "Sector", 
                "Nano_Cluster", "Cluster", "Nano Cluster", "Vendor", "BTS_VENDOR", 
                "BCCH_OR_TRX1_Freq", "LAC", "PCI", "TAC_4G"
            ]
            
            # Convert to uppercase for comparison
            allowed_columns_upper = [col.upper() for col in allowed_columns_raw]
    ...
    def generate_rawndb_csv(self, output_name):
        ...
        try:
            ...
            if 'EUtranCell' in output_df.columns:
                output_df['Sector'] = output_df['EUtranCell'].str.extract(r'(\d+)$')
                output_df['Sector'] = pd.to_numeric(output_df['Sector'], errors='coerce')
